<?php
    ...
    if (isset($postdata)) { // get user data
		...
		$email = $request->email;
		$passwds = $request->newpassword;
		
			
        		// verify if username existe and have the same email with the on the user enter
			$records = $db->prepare("SELECT email FROM users WHERE username = :username ");
			$records->bindParam(':username', $username, PDO::PARAM_STR);
			$records->execute();
			$results = $records->fetch(PDO::FETCH_ASSOC);	
				// if user name exist and th email matches
			...
			else{ // if the emails doesn't matches
				echo "don't matched" ;
